/**
 * THE Candlorre — CHECKOUT API ROUTE
 * POST /api/checkout
...
router.post('/', async (req, res) => {
  try {
    ...
    return sendSuccess(res, {
      ...
      totals,
      paymentMethod,
      razorpayKeyId: config.payments.razorpay.keyId || 'rzp_test_51PTheCandlorre'
...
router.post('/razorpay-order', async (req, res) => {
  try {
    ...
    if (keyId && keySecret && !keyId.includes('CandlorreDemo99') && !keySecret.includes('secret_key_88')) {
      try {
    ...
    return sendSuccess(res, {
      ...
      amount: amountInPaise,
      currency: 'INR',
      keyId: keyId || 'rzp_test_51PTheCandlorre',
      totals
